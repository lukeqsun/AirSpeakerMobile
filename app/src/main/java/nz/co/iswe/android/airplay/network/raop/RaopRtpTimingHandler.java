...
//	private static Logger LOG = Logger.getLogger(RaopRtpTimingHandler.class.getName());
	...
	private class TimingRequester implements Runnable {
		...
		public void run() {
			while ( ! Thread.currentThread().isInterrupted() ) {
				...
				LogManager.i("sending timingRequestPacket: " + timingRequestPacket);
	...
	public synchronized void startTimeSync(){
		...
		if (synchronizationThread != null && ! started) {
			...
			LogManager.i("Time synchronizer started");
			started = true;
		}
	}
	...
	private synchronized void timingResponseReceived(final RaopRtpPacket.TimingResponse timingResponsePacket) {
		...
		LogManager.i("Timing response with weight " + weight + " indicated offset " + remoteSecondsOffset + " thereby adjusting the averaged offset by " + secondsTimeAdjustment + " leading to the new averaged offset " + averageRemoteSecondsOffset.get());
	}

	private synchronized void syncReceived(final RaopRtpPacket.Sync syncPacket) {
		LogManager.i("sync received : " + syncPacket);
		if ( ! averageRemoteSecondsOffset.isEmpty() ) {
			...
			LogManager.w("Times synchronized, cannot correct latency of sync packet");
		}
	}
