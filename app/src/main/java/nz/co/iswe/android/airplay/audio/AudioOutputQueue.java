...
public class AudioOutputQueue implements AudioClock {
	...
	Context context ;
	public AudioOutputQueue(final AudioStreamInformationProvider streamInfoProvider,Context context) {
...
//		channelConfig = streamInfoProvider.getChannels();
		...
		//create the AudioTrack    3  44100  2  2 16384 1
		audioTrack = new AudioTrack(streamType, sampleRateInHz, channelConfig, audioFormat, bufferSizeInBytes, mode);
		//3 44100 3 2 16384 1
//		audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, 44100, AudioFormat.CHANNEL_CONFIGURATION_STEREO ,  AudioFormat.ENCODING_PCM_16BIT, bufferSizeInBytes, AudioTrack.MODE_STREAM);
	...
	private class EnQueuer implements Runnable {
		...
		public void run() {
			try {
				...
				audioTrack.play();
				MyApplication.getInstance().setPlayerState(TransportState.PLAYING);
...
//			}
			...
			if (convertUnsignedToSigned) {
				...
				for(int i=0; i < samplesConverted.length; i += 2){
					samplesConverted[i] = (byte)((samplesConverted[i] & 0xff) - 0x80);
					//add by ville [convert litter-endian to big-endian]
		        ...
		        // 大端转小端，小端转大端也是一样的，注意是PCM16
//	            for (int i = 0; i < samplesConverted.length / 2; i++) {
//	                    byte tmp = samplesConverted[i * 2];
//	                    samplesConverted[i * 2] = samplesConverted[i * 2 + 1];
//	                    samplesConverted[i * 2 + 1] = tmp;
//	            }
			}
...
//		context.unregisterReceiver(receiver);
	...
	BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
            ...
            if(com.fenggit.util.Constants.IKEY_MEDIA_PLAY_OR_PAUSE.equals(intent.getAction())){
                audioTrack.pause() ;
            }
